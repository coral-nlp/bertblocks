...
class PolyBertEmbeddings(nn.Module):
    ...
    def __init__(self, config: "PolyBertConfig"):
        ...
        match config.pos_emb_kind:
            case "sinusoidal":
                self.pos = SinusoidalPositionalEncoding(config.hidden_size, config.max_sequence_length)
            case "learned":
                self.pos = LearnedPositionalEncoding(config.hidden_size, config.max_sequence_length)
            case _:
