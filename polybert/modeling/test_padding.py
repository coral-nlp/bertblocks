...
from polybert.modeling.padding import (
    get_block_mask_mod,
    index_first_axis,
...
class TestUnpadSequenceInput:
    """Test the unpad_sequence function."""

    def test_basic_unpadding(self) -> None:
        """Test basic unpadding functionality."""
        ...
        unpacked, indices, cu_seqlens, max_seq_len = unpad_sequence(hidden_states, attention_mask)
    ...
    def test_no_padding(self) -> None:
        ...
        packed, indices, cu_seqlens, max_seq_len = unpad_sequence(hidden_states, attention_mask)
    ...
    def test_different_dtypes(self) -> None:
        ...
        packed_f32, indices, cu_seqlens, max_seq_len = unpad_sequence(hidden_states_f32, attention_mask)
        assert packed_f32.dtype == torch.float32

        # Test with float64
        hidden_states_f64 = hidden_states_f32.double()
        packed_f64, _, _, _ = unpad_sequence(hidden_states_f64, attention_mask)
        assert packed_f64.dtype == torch.float64


class TestPadSequence:
    """Test the pad_sequence function."""
    ...
    def test_basic_padding(self) -> None:
        ...
        padded = pad_sequence(hidden_states, indices, batch_size, seq_len)
    ...
    def test_full_sequences(self) -> None:
...
class TestUnpadRoundTrip:
    """Test that unpad_sequence and pad_sequence are inverse operations."""
    ...
    def test_roundtrip(self) -> None:
        ...
        # Unpad
        packed, indices, cu_seqlens, max_seq_len = unpad_sequence(original_hidden_states, attention_mask)

        # Pad back
        reconstructed = pad_sequence(packed, indices, batch_size, seq_len)
    ...
    def test_roundtrip_no_padding(self) -> None:
        ...
        # Unpad
        unpacked, indices, cu_seqlens, max_seq_len = unpad_sequence(original_hidden_states, attention_mask)

        # Pad back
        reconstructed = pad_sequence(unpacked, indices, batch_size, seq_len)
...
class TestBlockMask:
    """Test the document block masking."""

    def test_block_mask(self) -> None:
        """Test that block mask works correctly."""
        cu_seqlens = torch.Tensor([128, 256, 384, 512])
        mask = get_block_mask_mod(cu_seqlens)
        mask = create_block_mask(mask, None, None, 512, 512, device="cpu")
        assert torch.allclose(mask.to_dense(), torch.eye(4, dtype=torch.int))
