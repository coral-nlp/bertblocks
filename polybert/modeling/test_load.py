...
from polybert.modeling.load import from_bert_model, from_modernbert_model
...
class TestFromModernBertModel:
    """Test that huggingface ModernBERT and loaded polybert BERT implementations return same output."""

    def test_from_modernbert_model_single_sequence(self) -> None:
        """Test for a single given string."""
        from transformers import AutoModel, AutoTokenizer

        pretrained_model_name_or_path = "answerdotai/ModernBERT-base"

        poly_model = from_modernbert_model(pretrained_model_name_or_path)
        bert_model = AutoModel.from_pretrained(pretrained_model_name_or_path)
        poly_model.eval()
        bert_model.eval()
    ...
    def test_from_bert_model_multiple_sequences(self) -> None:
        ...
        tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path)
        seq = tokenizer(["I like cats.", "I like dogs."], return_tensors="pt", padding="max_length")
