"""Tensor packing and unpacking utilities for efficient attention computation.

This module provides utilities for packing and unpacking variable-length sequences
into dense tensors, enabling efficient computation with padded sequences.
...
def pack_seq(
    hidden_states: "torch.FloatTensor", attention_mask: "torch.LongTensor | None"
) -> tuple["torch.FloatTensor", "torch.Tensor", "torch.Tensor", int]:
    """Pack variable-length sequences into a dense tensor by removing padding.

    This function takes batched sequences with padding and packs them into a single
    dense tensor containing only the valid (non-padded) tokens. This is useful for
    efficient computation in attention mechanisms where padding tokens should be ignored.

    Args:
        hidden_states: Input tensor of shape (batch_size, seq_len, ...) containing
            the sequence representations.
        attention_mask: Optional mask tensor of shape (batch_size, seq_len) where
            1 indicates valid tokens and 0 indicates padding. If None, all tokens
            are considered valid.

    Returns:
        tuple containing:
        - packed_hidden_states: Tensor of shape (total_valid_tokens, ...) containing
    ...
    Example:
        ...
        >>> packed, indices, cu_seqlens, max_len = pack_seq(hidden_states, attention_mask)
...
def unpack_seq(
    ...
    """Unpack a dense tensor back into batched sequences with padding.

    This function reverses the packing operation, taking a dense tensor of valid
    tokens and reconstructing the original batched format with appropriate padding.
    Padded positions will contain zeros.

    Args:
        hidden_states: Dense tensor of shape (total_valid_tokens, ...) containing
            the processed representations of valid tokens.
        indices: 1D tensor of length total_valid_tokens containing the flattened
            indices where each token should be placed in the unpacked tensor.
        batch: Batch size for the output tensor.
        seqlen: Maximum sequence length for the output tensor.
    ...
    Example:
        ...
        >>> unpacked = unpack_seq(packed, indices, batch=2, seqlen=4)
        >>> unpacked.shape  # torch.Size([2, 4, 768]) - back to original shape
        # Padding positions will be zero-filled
...
def doc_mask(cu_seqlens: torch.Tensor) -> _mask_mod_signature:
