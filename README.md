...
This project implements **PolyBert**, a highly configurable transformer encoder codebase that allows experimentation with various architectural components including:
...
- **Positional Encodings**: ALiBi, Sinusoidal, RoPE, ...
...
The architecture is highly configurable through the `PolyBertConfig` class. Key parameters include:

```python
from polybert import PolyBertConfig, PolyBertForMaskedLM
...
model = PolyBertForMaskedLM(config)
```
